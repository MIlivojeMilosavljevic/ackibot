...
void set_dir(u8 motor_idx, dir_t dir){
	switch(motor_idx){
		case 0:
			if(dir == CW){
...
void set_pwm(u8 motor_idx, u16 s){
	switch(motor_idx){
		case 0:
			tc1.ocra = s;
			break;
...
void ramp_speed(u8 i) {
	speed_t target = voting_read(speed_i[i]);
#if 0
	speed_o[i] = target;
#else
	speed_o[i] = clamp(
		target,
		speed_o[i] - speed_ramp_delta,
		speed_o[i] + speed_ramp_delta
	);
#endif
	set_motor(i);
...
void set_target_speed(u8 i, i16 val) {
...
ISR(PCINT1_vect) {
	...
	i8 inc0 = enc_ba_to_pos_inc[prev.ba0][curr.ba0];
